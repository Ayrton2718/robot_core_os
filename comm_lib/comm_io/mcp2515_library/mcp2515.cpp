...
uint8_t MCP2515::readRegister(const REGISTER reg)
{
    ...
    send_buff[0] = INSTRUCTION_READ;
    send_buff[1] = reg;
    send_buff[2] = 0x00;

    m_spi.read_write(send_buff, read_buff, 3);
...
{
    uint8_t send_buff[2 + n];
    uint8_t read_buff[2 + n];
    
    send_buff[0] = INSTRUCTION_READ;
    send_buff[1] = reg;
    memset(&send_buff[2], 0x00, sizeof(uint8_t) * n);

    m_spi.read_write(send_buff, read_buff, 2 + n);

    memcpy(values, &read_buff[2], sizeof(uint8_t) * n);
}
...
void MCP2515::setRegister(const REGISTER reg, const uint8_t value)
{
    ...
    send_buff[0] = INSTRUCTION_WRITE;
    send_buff[1] = reg;
    send_buff[2] = value;

    m_spi.write(send_buff, 3);
}
...
void MCP2515::setRegisters(const REGISTER reg, const uint8_t values[], const uint8_t n)
{
    ...
    send_buff[0] = INSTRUCTION_WRITE;
    send_buff[1] = reg;
    memcpy(&send_buff[2], values, sizeof(uint8_t) * n);

    m_spi.write(send_buff, 2 + n);
}
...
void MCP2515::modifyRegister(const REGISTER reg, const uint8_t mask, const uint8_t data)
{
    ...
    send_buff[0] = INSTRUCTION_BITMOD;
    send_buff[1] = reg;
    send_buff[2] = mask;
    send_buff[3] = data;

    m_spi.write(send_buff, 4);
}
...
uint8_t MCP2515::getStatus(void)
{
    ...
    send_buff[0] = INSTRUCTION_READ_STATUS;
    send_buff[1] = 0x00;

    m_spi.read_write(send_buff, read_buff, 2);

    return read_buff[1];
}

void MCP2515::mcpReset(void)
{
    uint8_t send_byte = INSTRUCTION_RESET;

    m_spi.write(&send_byte, 1);
}
...
MCP2515::ERROR MCP2515::setMode(const CANCTRL_REQOP_MODE mode)
{
    ...
    uint64_t endTime = m_spi.now_time() + 10;
    bool modeMatch = false;
    while(m_spi.now_time() < endTime) {
